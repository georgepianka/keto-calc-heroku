...
const NavBar = ({ currentUser, loggedIn, toggle, isOpen }) => {
  ...
  return (
    ...
    <Navbar className="mb-5" style={{backgroundColor:"rgba(98, 221, 189, .8)"}} light expand="md">
        <NavbarBrand>
        <NavLink style={{textDecoration: "none" }} exact to="/calendar/days">
        ...
        <Collapse isOpen={isOpen} navbar>
            <Nav className="ml-auto" navbar>
                <NavItem>
                    <NavLink className="btn btn-success btn-sm border border-muted text-light" exact to='/log/days/new' activeStyle={{fontSize: '25px'}}>Log</NavLink>
                </NavItem>
                <NavItem>
